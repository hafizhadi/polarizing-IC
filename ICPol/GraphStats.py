...
def AnalyzeGraph(graph):
    ...
    q = [0] * graph.order()
    accCount = [0] * graph.order()
    propCount = [0] * graph.order()

    # Collect node data
    for n, d in graph.nodes_iter(data=True):
        q[n] = d["q"]
        accCount[n] = d["accCount"]
        propCount[n] = d["propCount"]
...
def VisualizeGraph(graph, filename, mode):
    ...
    plt.annotate("mean= " + str(nCount.mean()) + "\nsize=" + str(len(nCount)), xy=(nCount.mean(), 0.8),
    ...
    plt.annotate("mean= " + str(eDegree.mean()) + "\nsize=" + str(len(eDegree)), xy=(eDegree.mean(), 0.8),
                 xycoords=('data', 'axes fraction'),
                 xytext=(10, 0), textcoords='offset points')
    ...
    if (mode == 'IC'):
        ...
        plt.annotate("mean= " + str(nProb.mean()) + "\nsize=" + str(len(nProb)), xy=(nProb.mean(), 0.8),
                     xycoords=('data', 'axes fraction'), xytext=(10, 0), textcoords='offset points')
        ...
        plt.annotate("mean= " + str(eProb.mean()) + "\nsize=" + str(len(eProb)), xy=(eProb.mean(), 0.8),
                     xycoords=('data', 'axes fraction'),
                     xytext=(10, 0), textcoords='offset points')
